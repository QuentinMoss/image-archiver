...
for i in "$image_path"/*; do # per each subdirectory in images dir...
	if [[ ! $(find "$i" -type f -mtime -30) ]]; then # if we find no recently modified files in that subdir...
		...
		rsync -va --stats --remove-source-files "$i" "$archive_path" && rsync_success=1 || rsync_success=0
	fi
	if [ ! $rsync_success ]; then # if an rsync failed, we leave before breaking more things
